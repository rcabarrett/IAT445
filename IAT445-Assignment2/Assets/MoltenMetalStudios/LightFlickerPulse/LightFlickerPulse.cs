            ...
            if (((!error1 && !useOutsideLight) || (!error2 && useOutsideLight)) && assigned && useType != UseType.Off && Time.timeScale > timescaleMin)
            {
                ...
                if (useOutsideLight)
                {
                    for (int i = 0; i < outsideLight.Count; i++)
                    {
                        ...
                        {
                            outsideLight[i].intensity = Mathf.Clamp(mainOutValueI[i], 0, 100);
                        }
                ...
                else
                {
                    ...
                    {
                    lightS.intensity = Mathf.Clamp(mainValueI * z, 0, 100);
                    }
                    else if (useType == UseType.Range)
                    {
...
// Developed and distributed by Jayden Tobar-Chapman.
